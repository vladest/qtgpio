...
ICM_20948_Status_e ICM_20948::setBiasGyroX(int32_t newValue)
{
    if (_device._dmp_firmware_available == true) // Is DMP supported?
    {
        ...
        status = inv_icm20948_write_mems(&_device,
                                         GYRO_BIAS_X,
                                         4,
...
const char *ICM_20948::statusString(ICM_20948_Status_e stat)
{
    ...
    case ICM_20948_Stat_DMPNotSupported:
        return "DMP Firmware Not Supported. Is #define ICM_20948_USE_DMP commented in "
               "util/ICM_20948_C.h?";
        break;
...
ICM_20948_Status_e ICM_20948::checkID(void)
{
    ...
    if (status != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::checkID: ICM_20948_check_id returned: %s", statusString(status));
    }
    return status;
}
...
bool ICM_20948::isConnected(void)
{
    ...
    qDebug("ICM_20948::isConnected: checkID returned: %s", statusString(status));
...
ICM_20948_Status_e ICM_20948::setSampleMode(uint8_t sensor_id_bm, uint8_t lp_config_cycle_mode)
{
    ...
    QThread::msleep(1); // Give the ICM20948 time to change the sample mode (see issue #8)
    return status;
}
...
ICM_20948_Status_e ICM_20948::clearInterrupts(void)
{
    ...
    status = ICM_20948_execute_r(&_device,
                                 AGB0_REG_INT_STATUS,
                                 (uint8_t *) &int_stat,
...
uint8_t ICM_20948::i2cMasterSingleR(uint8_t addr, uint8_t reg)
{
    ...
    if (status != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::i2cMasterSingleR: ICM_20948_i2c_master_single_r returned: %s", statusString(status));
    }
    return data;
}
...
ICM_20948_Status_e ICM_20948::startupDefault(bool minimal)
{
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: checkID returned: %s", statusString(retval));
    ...
    retval = swReset();
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: swReset returned: %s", statusString(retval));
    ...
    retval = sleep(false);
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: sleep returned: %s", statusString(retval));
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: lowPower returned: %s", statusString(retval));
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: startupMagnetometer returned: %s", statusString(retval));
    ...
    {
        qDebug("ICM_20948::startupDefault: minimal startup complete!");
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: setSampleMode returned: %s", statusString(retval));
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: setFullScale returned: %s", statusString(retval));
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: setDLPFcfg returned: %s", statusString(retval));
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: enableDLPF (Acc) returned: %s", statusString(retval));
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupDefault: enableDLPF (Gyr) returned: %s", statusString(retval));
...
ICM_20948_Status_e ICM_20948::enableDMPSensor(enum inv_icm20948_sensor sensor, bool enable)
{
    if (_device._dmp_firmware_available == true) // Should we attempt to enable the sensor?
    {
        ...
        qDebug("ICM_20948::enableDMPSensor:  _enabled_Android_0: %i", (int) _device._enabled_Android_0);
        qDebug("  _enabled_Android_1: %i", (int) _device._enabled_Android_1);
        qDebug("  _dataOutCtl1: %i", (int) _device._dataOutCtl1);
        qDebug("  _dataOutCtl2: %i", (int) _device._dataOutCtl2);
        qDebug("  _dataRdyStatus: %i", (int) _device._dataRdyStatus);
...
ICM_20948_Status_e ICM_20948::enableDMPSensorInt(enum inv_icm20948_sensor sensor, bool enable)
{
    if (_device._dmp_firmware_available == true) // Should we attempt to enable the sensor interrupt?
    {
        ...
        qDebug("ICM_20948::enableDMPSensorInt:  _enabled_Android_intr_0: %i", _device._enabled_Android_intr_0);
        qDebug("  _enabled_Android_intr_1: %i", _device._enabled_Android_intr_1);
        qDebug("  _dataIntrCtl: %i", (int) _device._dataIntrCtl);
...
ICM_20948_Status_e ICM_20948::setGyroSF(unsigned char div, int gyro_level)
{
    if (_device._dmp_firmware_available == true) // Should we attempt to set the Gyro SF?
    {
        ...
        qDebug("ICM_20948::setGyroSF:  pll: %i", (int) _device._gyroSFpll);
        qDebug("  Gyro SF is: %i", (int) _device._gyroSF);
...
ICM_20948_Status_e ICM_20948::initializeDMP(void)
{
    ...
    if (_device._dmp_firmware_available != true) {
        qDebug("ICM_20948::startupDMP: DMP is not available. Please check that you have "
    ...
    result = i2cControllerConfigurePeripheral(0,
    ...
    mySmplrt.g
        = 19; // ODR is computed as follows: 1.1 kHz/(1+GYRO_SMPLRT_DIV[7:0]). 19 = 55Hz. InvenSense Nucleo example uses 19 (0x13).
    mySmplrt.a
        = 19; // ODR is computed as follows: 1.125 kHz/(1+ACCEL_SMPLRT_DIV[11:0]). 19 = 56.25Hz. InvenSense Nucleo example uses 19 (0x13).
    //mySmplrt.g = 4; // 225Hz
    //mySmplrt.a = 4; // 225Hz
    //mySmplrt.g = 8; // 112Hz
    //mySmplrt.a = 8; // 112Hz
...
ICM_20948_Status_e ICM_20948::startupMagnetometer(bool minimal)
{
    ...
    if (tries == MAX_MAGNETOMETER_STARTS) {
        qDebug("ICM_20948::startupMagnetometer: reached MAX_MAGNETOMETER_STARTS (%i). Returning ICM_20948_Stat_WrongID", (int) MAX_MAGNETOMETER_STARTS);
        status = ICM_20948_Stat_WrongID;
        return status;
    } else {
        qDebug("ICM_20948::startupMagnetometer: successful magWhoIAm after %i", (int) tries);
        if (tries == 1)
            qDebug(" try");
        else
            qDebug(" tries");
    ...
    if (false) // Return now if minimal is true
    {
        qDebug("ICM_20948::startupMagnetometer: minimal startup complete!");
        return status;
    }
    ...
    reg.reserved_0
        = 0; // Make sure the unused bits are clear. Probably redundant, but prevents confusion when looking at the I2C traffic
    retval = writeMag(AK09916_REG_CNTL2, (uint8_t *) &reg);
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupMagnetometer: writeMag returned: %s", statusString(retval));

        status = retval;
        return status;
    }

    retval = i2cControllerConfigurePeripheral(0,
                                              MAG_AK09916_I2C_ADDR,
                                              AK09916_REG_ST1,
                                              9,
                                              true,
                                              ...
                                              false);
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::startupMagnetometer: i2cMasterConfigurePeripheral returned: %s", statusString(retval));
...
ICM_20948_Status_e ICM_20948::magWhoIAm(void)
{
    ...
    retval = status;
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::magWhoIAm: whoiam1: %i", whoiam1);
        qDebug(" (should be 72) readMag set status to: %s", statusString(retval));
    ...
    if (retval != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948::magWhoIAm: whoiam1: %i", whoiam1);
        qDebug(" (should be 72) whoiam2: %i", whoiam2);
        qDebug(" (should be 9) readMag set status to: %i", status);
    ...
    qDebug("ICM_20948::magWhoIAm: whoiam1: %i", whoiam1);
    qDebug(" (should be 72) whoiam2: %i", whoiam2);
    qDebug(" (should be 9). Returning ICM_20948_Stat_WrongID");
...
ICM_20948_Status_e ICM_20948_I2C::begin(QGpio *qgpio, bool ad0val, uint8_t ad0pin)
{
    // Associate
    _ad0 = ad0pin;
    m_qgpio = qgpio;
    ...
    qDebug() << "addr" << Qt::hex << _addr;
    m_qgpio->allocateI2CSlave(_addr, 1, 1, 40000);

    QGpioPort* adPort = nullptr;
    // Set pinmodes
    if (_ad0 != ICM_20948_ARD_UNUSED_PIN) {
        //pinMode(_ad0, OUTPUT);
        adPort = qgpio->allocateGpioPort(_ad0, QGpio::DIRECTION_OUTPUT).get();
        //digitalWrite(_ad0, _ad0val);
        adPort->setValue(_ad0val ? QGpio::VALUE_HIGH : QGpio::VALUE_LOW);
    }
    ...
    _device._last_bank
        = 255; // Initialize _last_bank. Make it invalid. It will be set by the first call of ICM_20948_set_bank.
    _device._last_mems_bank
        = 255; // Initialize _last_mems_bank. Make it invalid. It will be set by the first call of inv_icm20948_write_mems.
    ...
    status = startupDefault(/*_device._dmp_firmware_available*/true);
    if (status != ICM_20948_Stat_Ok) {
        qDebug("ICM_20948_I2C::begin: startupDefault returned: %s", statusString(status));
    }
    return status;
}
...
ICM_20948_Status_e ICM_20948_write_I2C(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
    ...
    QGpio* _gpio = ((ICM_20948_I2C *) user)->m_qgpio; // Cast user field to ICM_20948_I2C type and extract the I2C interface pointer
    ...
    auto size = _gpio->getI2CSlave(addr)->i2cWrite(reg, (const void*)data, len);
    // qDebug() << "ICM_20948_write_I2C. wrote" << size << "bytes out of" << len << "to" << Qt::hex << addr << "reg" << Qt::hex << reg;
    // for (uint8_t i = 0; i < len; i++) {
    //     qDebug().noquote() << Qt::hex << data[i];
    // }
    ...
    return size == len ? ICM_20948_Stat_Ok : ICM_20948_Stat_Err;
}
...
ICM_20948_Status_e ICM_20948_read_I2C(uint8_t reg, uint8_t *buff, uint32_t len, void *user)
{
    ...
    QGpio* _gpio = ((ICM_20948_I2C *) user)->m_qgpio;
    uint8_t addr = ((ICM_20948_I2C *) user)->_addr;
    if (_gpio == NULL) {
        return ICM_20948_Stat_ParamErr;
    }

    auto _i2c = _gpio->getI2CSlave(addr);
    _i2c->i2cWrite(reg, (uint8_t)0);
    // _i2c->beginTransmission(addr);
    // _i2c->write(reg);
    ...
    uint32_t num_received = _i2c->i2cRead(reg, buff, len);
    //qDebug() << "ICM_20948_read_I2C" << num_received << len << "at addr" << Qt::hex << addr << "reg" << Qt::hex << reg;
    if (num_received == len) {
        // for (uint8_t i = 0; i < num_received; i++) {
        //     qDebug().noquote() << Qt::hex << buff[i];
        // }
